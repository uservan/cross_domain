...
import collections
import time
from typing import Literal, List, Set, Callable
import json
...
def get_empty_table(table):
    empty_table = []
    for row in table:
        empty_table.append([row[0]] + ["correct answer" for i in range(1, len(row))])
    return empty_table

CONFIGS = {
    "easy": {"level": 2, "columns": 3, "rows": ROWS},
    "medium": {"level": 2, "columns": 4, "rows": ROWS},    
    "hard": {"level": 2, "columns": 5, "rows": ROWS},
}
...
def generator_zebra_logic_problem(rows=4, columns=5, level=18):
    ...
    n_attributes = rows # rows
    m_objects = columns - 1 # cols - 1
    ...
    t1 = time.monotonic()
    premises = generate_puzzle(table, level=level, minimal_conditions=True, max_seconds_for_minimizing=30)
    t2 = time.monotonic()
    indent = len(str(len(premises)))
    for i, premise in enumerate(premises, 1):
        i = str(i).rjust(indent)
        puzzle_str.append(f"{i}. {premise}")
    puzzle_str.append("Fill the following table to show your final answer.")
    puzzle_str.append(format_table_no_header(header, get_empty_table(table)))
    # print(format_table_no_header(header, get_empty_table(table)))
...
def get_difficulty_level(answer, rows):

    num_pipes = len(re.findall(r"\|", answer)) - 2 * rows
    if num_pipes <= 10:
        return "easy"
    elif num_pipes <= 15:
        return "medium"
...
def generate(count=100, difficulty='medium', language='en', split="train", **kwargs):
    ...
    with tqdm(total=count) as pbar:  
        while generated_count < count:
            problem = generator_zebra_logic_problem(**params)
            problem["difficulty_level"] = get_difficulty_level(problem["answer"], ROWS)
            if problem["difficulty_level"] == difficulty:
