...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        def normalize_reward_model(example):
            if "reward_model" in example and "ground_truth" in example["reward_model"]:
                gt = example["reward_model"]["ground_truth"]
                example["reward_model"]["ground_truth"] = pack_ground_truth(gt)
            return example
        dataframes = []
        for parquet_file in self.data_files:
            # read parquet files and cache
            dataframe = datasets.load_dataset("parquet", data_files=parquet_file)["train"]
            if "reward_model" in dataframe.column_names:
                dataframe = dataframe.map(
                    normalize_reward_model,
                    desc=f"Packing reward_model.ground_truth in {parquet_file}"
                )
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if len(self.config.get('prompt_suffix', '')) > 0:
                tag = self.config.prompt_suffix
                if "<think>" in raw_prompt[-30:]:
                    raw_prompt =raw_prompt + "\n</think>\n\n"
                else:
                    raw_prompt =raw_prompt + "<think>\n\n</think>\n\n"
                    # raw_prompt = raw_prompt+"<think>\n\n</think>\n\n"
                if tag != 'no_think':
                    raw_prompt = raw_prompt + f'<{tag}>'+'\nOkay' 
