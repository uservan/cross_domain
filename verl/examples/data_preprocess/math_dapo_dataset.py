...
if __name__ == "__main__":
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            ...
            question = question[0]['content'].split('\n\n')[1].strip()
    ...
    import random
    indices = random.sample(range(len(train_dataset)), k=7500)
    train_dataset = train_dataset.select(indices).map(make_map_fn("train"), with_indices=True)
    ...
    train_dataset.to_parquet(os.path.join(local_dir, "DAPO-Math-7k.parquet"))
    # Save one example as JSON for reference
    example = train_dataset[0]
    with open(os.path.join(local_dir, "DAPO-Math-7k.json"), "w") as f:
